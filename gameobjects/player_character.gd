...
func _physics_process(_delta):
	...
	if (not mouse_move_accumulation.is_equal_approx(Vector2.ZERO)):
		...
		camera.global_rotate(Vector3.UP, converted_movement.x * -1.0 * turn_speed)
		camera.global_rotate(camera.transform.basis.x, converted_movement.y * turn_speed * (1.0 if y_invert else -1.0))
